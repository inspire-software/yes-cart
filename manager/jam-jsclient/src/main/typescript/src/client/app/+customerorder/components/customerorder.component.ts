...
export class CustomerOrderComponent implements OnInit, OnDestroy {
  ...
  formUnbind():void {
    if (this.brandFormSub) {
      console.debug('CustomerOrderComponent unbining form');
      this.brandFormSub.unsubscribe();
    }
  }

  formChange():void {
    console.debug('CustomerOrderComponent validating formGroup is valid: ' + this.validForSave, this._brand);
  ...
  onMainDataChange(event:any) {
    console.debug('CustomerOrderComponent main data changed', this._brand);
    this.changed = true;
  }

  onAttributeDataChanged(event:any) {
    console.debug('CustomerOrderComponent attr data changed', this._brand);
    this.changed = true;
  ...
  protected onSelectRow(row:AttrValueBrandVO) {
    console.debug('CustomerOrderComponent onSelectRow handler', row);
