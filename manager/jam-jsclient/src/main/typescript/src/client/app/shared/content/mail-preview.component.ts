...
export class MailPreviewComponent implements OnInit, OnDestroy {
  ...
  onSelectClick(customerorder: CustomerOrderInfoVO) {
    ...
    let myWindow = window.open('/yes-manager/service/content/mail/' + this.selectedShop + '/' + this.selectedTemplate + '?order=' + customerorder.ordernum, 'PREVIEW', 'width=800,height=500');
  }
