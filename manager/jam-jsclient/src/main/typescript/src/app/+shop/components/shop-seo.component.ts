...
export class ShopSEOComponent implements OnInit, OnDestroy {
  ...
  public shopLocalization:ShopSeoVO;
  ...
  onSaveHandler() {
    ...
    if (this.shop.shopId > 0 && this.shopLocalization) {
      this.loading = true;
      this._shopService.saveShopSeo(this.shopLocalization).subscribe(shopLocalization => {
  ...
  onDiscardEvent() {
    ...
    if (this.shop.shopId > 0) {
      this.loading = true;
      this._shopService.getShopSeo(this.shop.shopId).subscribe(shopLocalization => {
