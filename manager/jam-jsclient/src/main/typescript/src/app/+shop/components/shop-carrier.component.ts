...
export class ShopCarrierComponent implements OnInit, OnDestroy {
  ...
  public shopCarriersVO:Array<ShopCarrierAndSlaVO>;
  public availableCarriers:Array<ShopCarrierAndSlaVO>;
  public selectedCarriers:Array<Pair<ShopCarrierAndSlaVO, ShopCarrierSlaVO>>;

  public changed:boolean = false;

  public newCarrier:CarrierInfoVO;
  @ViewChild('editNewCarrierName')
  private editNewCarrierName:ModalComponent;

  public editCarrierSla:ShopCarrierSlaVO;
  @ViewChild('editCarrierSlaRank')
  private editCarrierSlaRank:ModalComponent;

  public newCarrierForm:any;
  public validForSave:boolean = false;
  public editCarrierSlaForm:any;
  public validForEdit:boolean = false;
  ...
  onSaveHandler() {
    ...
    if (this.shop.shopId > 0 && this.shopCarriersVO) {
      this.loading = true;
      this._shippingService.saveShopCarriers(this.shopCarriersVO).subscribe(shopLanguagesVo => {
        this.changed = false;
        this._reload = false;
        this.loading = false;
        this.onRefreshHandler();
  ...
  onRefreshHandler() {
    ...
    if (this.shop.shopId > 0) {
      this.loading = true;
      this._shippingService.getShopCarriers(this.shop.shopId).subscribe(shopCarriersVo => {
