...
export class ShopCatalogMinComponent implements OnInit, OnDestroy {
  ...
  public existingShop:boolean = false;

  public newCategory:BasicCategoryVO;
  ...
  public assigned:Array<CategoryVO> = null;
  ...
  loadData(current:number = 0) {
    ...
    if (this.existingShop) {
      ...
      if (current > 0) {
        ...
        this._categoryService.getBranchCategories(current, []).subscribe(
          cats => {
        ...
        // expanding initial
        this.loading = true;
        this._shopService.getShopCategories(this.shop.shopId).subscribe(
          cats => {
            LogUtil.debug('ShopCatalogMinComponent assigned categories', cats);
            this.assigned = cats;
            let _assignedIds: Array<number> = this.adaptToIds(cats);

            this._categoryService.getBranchesCategoriesPaths(_assignedIds).subscribe(
              cats => {
                LogUtil.debug('ShopCatalogMinComponent loading branch path', cats);

                this._categoryService.getBranchCategories(current, cats).subscribe(
                  cats => {
  ...
  onSaveHandler() {
    ...
    if (this.shop.shopId > 0) {
      this._shopService.saveShopCategories(this.shop.shopId, this.assigned).subscribe(
        cats => {
          this.assigned = cats;
          this.changed = false;
        }
