...
export class ImpexDataDescriptorsComponent implements OnInit, OnDestroy {
  ...
  public viewMode:string = ImpexDataDescriptorsComponent.DESCRIPTORS;

  public datadescriptors:Array<DataDescriptorVO> = [];
  public datadescriptorFilter:string;
  public datadescriptorSort:Pair<string, boolean> = { first: 'name', second: false };

  public selectedDataDescriptor:DataDescriptorVO;

  public datadescriptorEdit:DataDescriptorVO;
  ...
  onDataDescriptorSelected(data:DataDescriptorVO) {
    LogUtil.debug('ImpexDataDescriptorsComponent onDataDescriptorSelected', data);
    this.selectedDataDescriptor = data;
  }

  onDataDescriptorChanged(event:FormValidationEvent<DataDescriptorVO>) {
  ...
  onRowEditDataDescriptor(row:DataDescriptorVO) {
  ...
  onSaveHandler() {
    ...
    if (this.validForSave && this.changed) {
      ...
      if (this.datadescriptorEdit != null) {
        ...
        this.loading = true;
        this._dataDroupService.saveDataDescriptor(this.datadescriptorEdit).subscribe(
          rez => {
  ...
  onDeleteConfirmationResult(modalresult: ModalResult) {
    ...
    if (ModalAction.POSITIVE === modalresult.action) {
      ...
      if (this.selectedDataDescriptor != null) {
        ...
        this.loading = true;
        this._dataDroupService.removeDataDescriptor(this.selectedDataDescriptor).subscribe(res => {
  ...
  private getAllDataDescriptors() {
    this.loading = true;
    this._dataDroupService.getAllDataDescriptors().subscribe( alldatadescriptors => {
