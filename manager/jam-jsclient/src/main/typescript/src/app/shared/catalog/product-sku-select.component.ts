...
@Component({
  selector: 'cw-product-sku-select',
...
export class ProductSkuSelectComponent implements OnInit, OnDestroy {
  ...
  public filteredProductSkus : ProductSkuVO[] = [];
  public productSkuFilter : string;
  ...
  public productSkuFilterRequired:boolean = true;
  public productSkuFilterCapped:boolean = false;
  ...
  private getAllProductSkus() {
    ...
    if (!this.productSkuFilterRequired) {
      ...
      this._productService.getFilteredProductSkus(_ctx).subscribe(allproducts => {
