...
import { ProductTypeInfoVO, SearchContextVO } from './../model/index';
...
@Component({
  selector: 'cw-product-type-select',
...
export class ProductTypeSelectComponent implements OnInit, OnDestroy {
  ...
  public filteredProductTypes : ProductTypeInfoVO[] = [];
  public productTypeFilter : string;
  ...
  public productTypeFilterRequired:boolean = true;
  public productTypeFilterCapped:boolean = false;
  ...
  private getAllProductTypes() {
    ...
    if (!this.productTypeFilterRequired) {
      ...
      this._typeService.getFilteredProductTypes(_ctx).subscribe(allproductTypes => {
