...
export class ErrorModalComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.errorSub = ErrorEventBus.getErrorEventBus().errorUpdated$.subscribe(event => {
      ...
      if (event !== 'init') {
        ...
        if (this.errorAuth && (
              UserEventBus.getUserEventBus().current() === null ||
              UserEventBus.getUserEventBus().currentJWT() === null ||
              UserEventBus.getUserEventBus().currentJWT().decoded === null ||
              UserEventBus.getUserEventBus().currentJWT().decoded.exp * 1000 < new Date().getTime()
            )
          ) {
  ...
  onErrorResult(modalresult: ModalResult) {
  ...
  private setErrorString(event:any) {
    ...
    if (message.message) {
      if (message.message == 'MODAL_ERROR_MESSAGE_AUTH') {
        ...
        } else if (message.message.includes('isTrusted')) {
