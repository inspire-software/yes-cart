...
@Component({
  selector: 'cw-content-min-select',
      ...
      if (cached) {
        ...
        if (current > 0) {
          if (!this.expandCurrent(this.nodes, current)) {
            ...
            this.loading = true;
            this._contentService.getShopBranchesContentPaths(this.shop.shopId, [ current ]).subscribe(
              cats => {
        ...
        if (current > 0 && !this.existsCurrent(this.nodes, current)) {
          ...
          this.loading = true;
          this._contentService.getShopBranchContent(this.shop.shopId, current, expanded.ALL).subscribe(
            cats => {
