...
export class PriceListComponent implements OnInit, OnDestroy {
  ...
  public filteredPricelist:Array<PriceListVO>;
  ...
  isAvailableFromNow(row:PriceListVO) {
    return row.salefrom === null || (row.salefrom < new Date());
  }

  isAvailableToNow(row:PriceListVO) {
    return row.saleto === null || (row.saleto > new Date());
  }

  getPolicyLabels(row:PriceListVO) {
    let out = '';
    if (row.ref) {
