...
export class InventoryComponent implements OnInit, OnDestroy {
  ...
  public filteredInventory:Array<InventoryVO>;
  ...
  isAvailableFromNow(row:InventoryVO) {
    return row.availablefrom === null || row.availability == 2 /* PreOrder */ || (row.availablefrom < new Date());
  }

  isAvailableToNow(row:InventoryVO) {
    return row.availableto === null || (row.availableto > new Date());
  }

  isReleasedNow(row:InventoryVO) {
