...
export class CatalogProductsComponent implements OnInit, OnDestroy {
  ...
  public viewMode:string = CatalogProductsComponent.PRODUCTS;

  public products:SearchResultVO<ProductVO>;
  public productFilter:string;
  public productFilterRequired:boolean = true;

  private delayedFilteringProduct:Future;
  private delayedFilteringProductMs:number = Config.UI_INPUT_DELAY;

  public selectedProduct:ProductVO;

  public productEditIsCopyOf:ProductVO;
  public productEdit:ProductWithLinksVO;
  public productEditAttributes:AttrValueProductVO[] = [];
  private productAttributesUpdate:Array<Pair<AttrValueProductVO, boolean>>;

  @ViewChild('deleteConfirmationModalDialog')
  private deleteConfirmationModalDialog:ModalComponent;

  public selectedSku:ProductSkuVO;

  public skuEditIsCopyOf:ProductSkuVO;
  public skuEdit:ProductSkuVO;
  public skuEditAttributes:AttrValueProductSkuVO[] = [];
  ...
  onRowEditProduct(row:ProductVO) {
    LogUtil.debug('CatalogProductsComponent onRowEditProduct handler', row);
    this.loading = true;
    this._pimService.getProductById(row.productId).subscribe(res => {
      ...
      if (this.productEdit.productId > 0) {
        this.loading = true;
        this._pimService.getProductAttributes(this.productEdit.productId).subscribe(attrs => {
  ...
  onRowEditSku(row:ProductSkuVO) {
    ...
    if (this.skuEdit.skuId > 0) {
      this.loading = true;
      this._pimService.getSKUAttributes(this.skuEdit.skuId).subscribe(attrs => {
  ...
  onRowCopyProduct(row:ProductVO) {
    ...
    if (this.validForSave && this.changed) {
      ...
      if (this.skuEdit != null) {
        ...
        if (!(this.skuEdit.skuId > 0) && this.skuEditIsCopyOf != null) {
          ...
          this._pimService.copySKU(this.skuEditIsCopyOf, this.skuEdit).subscribe(
            rez => {
              if (this.productEdit != null) {
          ...
          this._pimService.saveSKU(this.skuEdit).subscribe(
            rez => {
              ...
              if (pk > 0 && this.skuAttributesUpdate != null && this.skuAttributesUpdate.length > 0) {
                ...
                this.loading = true;
                this._pimService.saveSKUAttributes(this.skuAttributesUpdate).subscribe(rez => {
        ...
        if (!(this.productEdit.productId > 0) && this.productEditIsCopyOf != null) {
          ...
          this._pimService.copyProduct(this.productEditIsCopyOf, this.productEdit).subscribe(
            rez => {
          ...
          this._pimService.saveProduct(this.productEdit).subscribe(
            rez => {
              ...
              if (pk > 0 && this.productAttributesUpdate != null && this.productAttributesUpdate.length > 0) {
                ...
                this.loading = true;
                this._pimService.saveProductAttributes(this.productAttributesUpdate).subscribe(rez => {
  ...
  onDeleteConfirmationResult(modalresult: ModalResult) {
    ...
    if (ModalAction.POSITIVE === modalresult.action) {
      ...
      if (this.selectedSku != null) {
        ...
        this.loading = true;
        this._pimService.removeSKU(this.selectedSku).subscribe(res => {
        ...
        this.loading = true;
        this._pimService.removeProduct(this.selectedProduct).subscribe(res => {
  ...
  private getFilteredProducts() {
    ...
    if (!this.productFilterRequired) {
      ...
      this._pimService.getFilteredProducts(this.products.searchContext).subscribe(allproducts => {
