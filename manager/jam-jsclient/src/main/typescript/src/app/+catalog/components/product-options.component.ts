...
export class ProductOptionsComponent implements OnInit, OnChanges {
  ...
  public options:Array<ProductSkuVO> = null;

  private _masterObject:ProductWithLinksVO;

  //sorting
  public sortColumn:string = 'rank';
  ...
  public filteredObjectOptions:Array<ProductOptionVO>;
  ...
  public selectedRow:ProductOptionVO;
  public selectedOptionSku:ProductSkuVO;

  public optionToEdit:ProductOptionVO;
  ...
  onOptionSkuSelected(row:ProductSkuVO) {
    this.selectedOptionSku = row;
  }

  onSelectRow(row:ProductOptionVO) {
  ...
  onProductSelected(event:FormValidationEvent<ProductSkuVO>) {
  ...
  onOptionSkuRemove(event:Pair<string, string>) {
  ...
  getSkuName(row:ProductOptionVO):string {
