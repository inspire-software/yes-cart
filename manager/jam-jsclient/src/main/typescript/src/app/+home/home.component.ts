...
export class HomeComponent implements OnInit {
  ...
  public availableWidgets:DashboardWidgetInfoVO[] = [];
  public filteredAvailableWidgets:DashboardWidgetInfoVO[] = [];

  public widgetFilter:string = null;
  public selectedWidget:DashboardWidgetInfoVO = null;

  public selectedWidgets:DashboardWidgetInfoVO[] = [];
  ...
  onAddHandler() {
    ...
    this._dashboardService.getAvailableWidgets(lang).subscribe((widgets:DashboardWidgetInfoVO[]) => {
  ...
  updateDashboardSettings(update:DashboardWidgetInfoVO[], refresh:boolean = false) {
    ...
    let lang = I18nEventBus.getI18nEventBus().current();
    this._dashboardService.updateDashboardWidgets(update, lang).subscribe((value:boolean) => {

      LogUtil.debug('HomeComponent updateDashboardWidgets', value);
  ...
  private getWidgets() {
    ...
    let lang = I18nEventBus.getI18nEventBus().current();
    this._dashboardService.getDashboard(lang).subscribe((widgets: DashboardWidgetVO[]) => {
