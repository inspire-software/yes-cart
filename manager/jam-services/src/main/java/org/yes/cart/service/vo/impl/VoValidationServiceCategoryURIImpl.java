 ...
 * Time: 15:31
 */
public class VoValidationServiceCategoryURIImpl implements VoValidationService {
    ...
    public VoValidationResult validate(final VoValidationRequest request) {
        ...
        final String uri = request.getValue();
        if (uri == null) {
        ...
        if (VALID_URI.matcher(uri).matches()) {
            ...
            final Long catId = this.categoryService.findCategoryIdBySeoUri(uri);
