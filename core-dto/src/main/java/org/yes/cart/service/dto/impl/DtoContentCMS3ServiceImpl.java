...
import org.yes.cart.domain.entity.impl.AttrValueEntityContent;
 ...
 * Time: 14:26
 */
public class DtoContentCMS3ServiceImpl
    ...
    private final GenericDAO<AttrValueEntityContent, Long> attrValueEntityContentDao;
     ...
     */
    public DtoContentCMS3ServiceImpl(final DtoFactory dtoFactory,
                                     final GenericService<Content> contentGenericService,
                                     final DtoAttributeService dtoAttributeService,
                                     final GenericDAO<AttrValueEntityContent, Long> attrValueEntityContentDao,
        ...
        this.attrValueEntityContentDao = attrValueEntityContentDao;
            ...
            if (parentOrUri == null) {
                ...
                if ("@".equals(parentOrUri.getFirst())) {
                    ...
                    if (parentCatId != null) {
                        ...
                        SearchContext.JoinMode.OR.setMode(currentFilter);
                        currentFilter.put("contentId", Collections.singletonList(SearchContext.MatchMode.EQ.toParam(parentCatId)));
    ...
    public AttrValueDTO updateEntityAttributeValue(final AttrValueDTO attrValueDTO) {
        ...
        if (matcher.find()) {
            ...
            final Iterator<AttrValueContent> itOld = content.getAttributes().iterator();
            while (itOld.hasNext()) {
                final AttrValueContent old = itOld.next();
                if (old.getAttributeCode().startsWith(keyStart)) {
                    itOld.remove();
                    attrValueEntityContentDao.delete(old);
                }
            }
            int pos = 0;
            int chunkCount = 1;
            ...
            do {
                ...
                AttrValueContent valueEntityContent = getPersistenceEntityFactory().getByIface(AttrValueContent.class);
                valueEntityContent.setAttributeCode(keyStart + '_' + chunkCount);
                valueEntityContent.setContent(content);
                valueEntityContent.setVal(part);
                attrValueEntityContentDao.create((AttrValueEntityContent) valueEntityContent);
                chunkCount++;
                pos += CHUNK_SIZE;
            } while (pos < val.length());

        } else {
            final AttrValueEntityContent valueEntityContent = attrValueEntityContentDao.findById(attrValueDTO.getAttrvalueId());
            attrValueAssembler.assembleEntity(attrValueDTO, valueEntityContent, getAdaptersRepository(), dtoFactory);
            attrValueEntityContentDao.update(valueEntityContent);
        }
        return attrValueDTO;
    ...
    public AttrValueDTO createEntityAttributeValue(final AttrValueDTO attrValueDTO) {
        ...
        AttrValueContent valueEntityContent = getPersistenceEntityFactory().getByIface(AttrValueContent.class);
        attrValueAssembler.assembleEntity(attrValueDTO, valueEntityContent, getAdaptersRepository(), dtoFactory);
        valueEntityContent.setAttributeCode(atr.getCode());
        valueEntityContent.setContent(content);
        valueEntityContent = attrValueEntityContentDao.create((AttrValueEntityContent) valueEntityContent);
        attrValueDTO.setAttrvalueId(valueEntityContent.getAttrvalueId());
        return attrValueDTO;
