...
public class CookiePolicyInclude extends BaseComponent {
        ...
        if (!alreadyAccepted) {
            ...
            if (cookiePolicyEnabled) {
                ...
                content = contentServiceFacade.getDynamicContentBody(
                        "cookie_policy_include", ShopCodeContext.getShopId(), getLocale().getLanguage(), Collections.EMPTY_MAP);
                if (content == null) {
                    content = ShopCodeContext.getShopId() + "_cookie_policy_include";
                }
                setVisible(true);
    ...
    private boolean determineIfCookiePolicyIsAlreadyAccepted() {
        ...
        final HttpServletRequest request = WicketUtil.getHttpServletRequest();
