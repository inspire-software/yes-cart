...
public class CustomerOrderServiceImpl extends BaseGenericServiceImpl<CustomerOrder> implements CustomerOrderService {
     ...
     */
    @Override
    public int findAwaitingDeliveriesCount(final Set<Long> shops, final String deliveryStatus, final List<String> orderStatus) {

        if (CollectionUtils.isNotEmpty(shops)) {

            return (customerOrderDeliveryDao.findByNamedQuery("DELIVERIES.IDS.WAITING.FOR.INVENTORY.BY.SHOP",
                    deliveryStatus,
                    orderStatus, shops)).size();
     ...
     */
    @Override
    public List<Long> findDeliveriesIds(final List<String> skuCodes) {

        final List<Long> waitingDeliveries;

        if (CollectionUtils.isNotEmpty(skuCodes)) {
            waitingDeliveries = (List) customerOrderDeliveryDao.findQueryObjectByNamedQuery("DELIVERIES.IDS.FOR.INVENTORY.BY.SKU",
                    skuCodes);
        } else {
