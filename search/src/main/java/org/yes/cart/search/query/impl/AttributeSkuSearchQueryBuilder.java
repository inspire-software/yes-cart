 ...
 * Time: 23:26
 */
public class AttributeSkuSearchQueryBuilder extends AbstractSearchQueryBuilderImpl implements ProductSearchQueryBuilder {
    ...
    public Query createStrictQuery(final long shopId, final String parameter, final Object value) {
        ...
        if (searchValue.contains(Constants.RANGE_NAVIGATION_DELIMITER)) { // value range navigation
            ...
            aggregatedQuery.add(createRangeQuery(escapedParameter + "_range", searchValueLo, searchValueHi, 3.5f), BooleanClause.Occur.SHOULD);
        ...
        aggregatedQuery.add(createTermQuery("facet_" + escapedParameter, ftSearchValue.toLowerCase(), 3.5f), BooleanClause.Occur.SHOULD);
