...
public class TestOrderAssemblerImpl extends BaseCoreDBTestCase {
    ...
    public static ShoppingCart getShoppingCart(final ApplicationContext context, final String customerEmail) {
        ...
        new SetShopCartCommandImpl(context, singletonMap(SetShopCartCommandImpl.CMD_KEY, 10))
                .execute(shoppingCart);
        new ChangeCurrencyEventCommandImpl(context, singletonMap(ChangeCurrencyEventCommandImpl.CMD_KEY, "USD"))
                .execute(shoppingCart);
    ...
    public static Customer createCustomer(final ApplicationContext context, final String prefix) {
        ShopService shopService = (ShopService) context.getBean(ServiceSpringKeys.SHOP_SERVICE);
        CustomerService customerService = (CustomerService) context.getBean(ServiceSpringKeys.CUSTOMER_SERVICE);
        AttributeService attributeService = (AttributeService) context.getBean(ServiceSpringKeys.ATTRIBUTE_SERVICE);
        AddressService addressService = (AddressService) context.getBean(ServiceSpringKeys.ADDRESS_SERVICE);
