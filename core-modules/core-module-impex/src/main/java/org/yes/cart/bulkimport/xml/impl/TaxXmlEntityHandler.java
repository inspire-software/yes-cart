...
public class TaxXmlEntityHandler extends AbstractXmlEntityHandler<TaxType, Tax> implements XmlEntityImportHandler<TaxType, Tax> {
    ...
    @Override
    protected void delete(final Tax tax) {
        this.taxService.delete(tax);
        this.taxService.getGenericDao().flush();
    }

    @Override
    protected void saveOrUpdate(final Tax domain, final TaxType xmlType, final EntityImportModeType mode) {
    ...
    @Override
    protected Tax getOrCreate(final TaxType xmlType) {
