 ...
 * Time: 22:23
 */
public class CountryStateXmlEntityHandler extends AbstractXmlEntityHandler<CountryStateType, State> implements XmlEntityImportHandler<CountryStateType> {
    ...
    @Override
    protected void delete(final State state) {
        this.stateService.delete(state);
        this.stateService.getGenericDao().flush();
    }

    @Override
    protected void saveOrUpdate(final State domain, final CountryStateType xmlType, final EntityImportModeType mode) {
    ...
    @Override
    protected State getOrCreate(final CountryStateType xmlType) {
