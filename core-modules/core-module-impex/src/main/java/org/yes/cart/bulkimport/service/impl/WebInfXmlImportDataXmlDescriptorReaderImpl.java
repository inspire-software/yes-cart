 ...
 * Time: 14:04
 */
public class WebInfXmlImportDataXmlDescriptorReaderImpl implements DataDescriptorReader<ImportDescriptor>, ApplicationContextAware {
    ...
    public WebInfXmlImportDataXmlDescriptorReaderImpl(final String pathToImportDescriptors) {
    ...
    public ImportDescriptor toDescriptorObject(final DataDescriptor dataDescriptor) {
        if (supports(dataDescriptor)) {
            ...
            try {
                final ImportDescriptor descriptor = getImportDescriptorFromXML(res.getInputStream());
